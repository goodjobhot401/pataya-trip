...
export function renderPersonalSettlement(expenses, userId) {
    const container = document.getElementById('personal-settlement');
    if (!container) return;

    const summary = getPersonalSummaries(expenses, userId);
    ...
    container.innerHTML = `
        ...
        <p class="settle-tip">💡 數值為正代表你有代墊，最後應收回；負數代表你應支付給他人。</p>
    `;
}
